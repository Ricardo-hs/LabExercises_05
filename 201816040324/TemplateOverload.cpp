...
int printArray(const T *array,int lowSubscrip, int highSubscript)
{
   ...
   if ( lowSubscrip>=highSubscript||lowSubscrip<0/* Write conditions to test if the size if negative,
   ...
   // display array
   for ( int i=lowSubscrip;i<lowSubscrip+highSubscript;i++/* Write code to iterate from lowSubscript up to
...
int main()
{
   ...
   elements =printArray(a, 0, 5) ; /* Write a call to printArray that specifies
   ...
   elements = printArray(c, 0, 5) ;/* Write a call to printArray that specifies
