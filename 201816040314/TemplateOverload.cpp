...
/* Write a header for an overloaded printArray function
   ...
   header */
template< typename T >
int printArray(const T *array,const int & lowSubscrip,const int & highSubscript)
{
   // check if subscript is negative or out of range
   if (highSubscript<0||lowSubscrip<0||highSubscript<=lowSubscrip) /* Write conditions to test if the size if negative,or if the range is invalid */
      return 0;

   int count = 0;

   // display array
   for ( size_t i=lowSubscrip;i<=highSubscript;++i ) /* Write code to iterate from lowSubscript up toand including highSubscript */
   {
      ++count;
      cout << array[ i ] << ' ';
   } // end for
