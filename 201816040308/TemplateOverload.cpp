...
/* Write a header for an overloaded printArray function
   ...
   header */
template< typename T >
int printArray( const T *array, int cont, int lowSubscrip, int highSubscript )
{
   // check if subscript is negative or out of range
   if ( lowSubscrip<0||lowSubscrip>=cont||highSubscript<0||highSubscript>=cont||lowSubscrip>=highSubscript/* Write conditions to test if the size if negative,
