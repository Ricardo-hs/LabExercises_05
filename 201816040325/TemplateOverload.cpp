...
/* Write a header for an overloaded printArray function
   ...
   header */
template< typename T >
int printArray( const T *array, int n,int lowSubscrip,int highSubscrip)
{
   // check if subscript is negative or out of range
   if ( /* Write conditions to test if the size if negative,
           or if the range is invalid */ (lowSubscrip<0 || highSubscrip<0)||(lowSubscrip>n||highSubscrip>n))
           {
               invalid_argument("LowSubscrip and highSubscrip should be more than 0");
               return 0;
           }
