...
template<typename T>
int  printArray(const  T *array,int lowSebscrip,int highSubscript)
{
   // check if subscript is negative or out of range
   if ( highSubscript<0||highSubscript-lowSebscrip<=0||lowSebscrip<0 )
      return 0;

   int count = 0;

   // display array
   for ( int i=lowSebscrip;i<=highSubscript;++i )
   {
      ++count;
      cout << array[ i ] << ' ';
   } // end for
