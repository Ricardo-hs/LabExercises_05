...
/* Write a header for an overloaded printArray function
   ...
   header */
template<typename T>
int printArray(const T *array,int a,int lowSubscrip,int highSunbscript)
{
   // check if subscript is negative or out of range
   if ( /* Write conditions to test if the size if negative,
           or if the range is invalid */ highSunbscript<=lowSubscrip||lowSubscrip<0)
      return 0;
   ...
   for ( /* Write code to iterate from lowSubscript up to
            and including highSubscript */int i = lowSubscrip; i <= highSunbscript; i++ )
   {
      ++count;
      cout << array[ i ] << ' ';
   } // end for
