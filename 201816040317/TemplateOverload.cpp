...
int printArray( const T *array, int lowSubscrip, int highSubscript )
{
   ...
   if (lowSubscrip<0||highSubscript<0||lowSubscrip>highSubscript/* Write conditions to test if the size if negative,
...
int main()
{
   ...
   elements = printArray( b, 0, ACOUNT - 1 );/* Write a call to printArray that specifies
                 -1 to 10 as the range */
   cout << elements << " elements were output\n\n";
